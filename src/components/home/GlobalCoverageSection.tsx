      ...
      <div className="container mx-auto px-6 max-w-5xl relative z-10 mb-12">
        ...
        <div className="text-center mb-12">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center space-x-2 py-1.5 px-4 rounded-full border border-blue-500/30 bg-slate-50 mb-6 shadow-[0_0_15px_rgba(6,182,212,0.15)]"
          >
          ...
          >
            ...
            <span className="bg-gradient-to-r from-blue-400 via-purple-400 to-blue-400 text-transparent bg-clip-text block">
              150+ COUNTRIES
            </span>
          </motion.h2>
          ...
          {REGIONS.map((region) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-[10px] md:text-xs font-bold tracking-widest uppercase transition-all duration-300 border ${
                activeRegion === region
                  ? "bg-blue-600/30 text-blue-200 border-blue-400/60 shadow-[0_0_20px_rgba(79,70,229,0.3)]"
                  : "bg-white text-slate-600 border-black/10 hover:border-black/20 hover:text-slate-800"
              }`}
            >
              {region === "ALL" && activeRegion === "ALL" && (
                <Globe className="w-3 h-3 inline mr-2 text-blue-400 mb-0.5" />
              )}
              {region}
            </button>
          ))}
          ...
          <AnimatePresence>
            {visibleCountries.map((country) => (
              <motion.div
                ...
                className="bg-slate-50/90 border border-black/10 hover:border-blue-500/40 hover:bg-white rounded-xl px-3.5 py-2.5 flex items-center gap-3 transition-all cursor-default group shadow-sm"
              >
                <img
                  src={`https://flagcdn.com/w40/${country.code}.webp`}
        ...
        {filteredCountries.length > 24 && !isExpanded && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-center mt-8">
            <button
              ...
              className="flex items-center space-x-2 px-6 py-2.5 rounded-full border border-black/15 bg-slate-50 text-xs font-bold text-slate-700 hover:text-black hover:border-blue-500/40 transition-colors shadow-md"
            >
