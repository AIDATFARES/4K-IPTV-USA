...
const benefits = [
  { icon: Bolt, title: "Instant Activation", text: "Your subscription is activated within minutes.", color: "text-blue-400" },
  { icon: Smartphone, title: "All Devices Supported", text: "Smart TV, Android, iOS, Mac, Fire Stick, PC, and more.", color: "text-sky-400" },
  { icon: Globe2, title: "Works Worldwide", text: "Enjoy 4K IPTV USA wherever you are, on any connection.", color: "text-blue-400" },
  { icon: ShieldCheck, title: "99.9% Uptime", text: "Reliable servers with stable, buffer-free streaming.", color: "text-blue-300" },
];

export default function Why4KIPTVUSASection() {
  return <section className="relative overflow-hidden bg-white py-24 sm:py-28">
    ...
    <div className="relative mx-auto max-w-[1120px] px-5 sm:px-8">
      <div className="mx-auto mb-14 max-w-3xl text-center">
        <span className="inline-flex rounded-full border border-blue-400/35 bg-blue-400/[0.06] px-4 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.18em] text-blue-300">Why choose 4K IPTV USA?</span>
        <h2 className="mt-6 text-4xl font-black uppercase leading-[0.98] tracking-tight text-black sm:text-5xl"><span className="block">The Ultimate</span><span className="mt-1 block bg-gradient-to-r from-blue-400 via-blue-400 to-blue-400 bg-clip-text text-transparent">4K IPTV USA Experience</span></h2>
      ...
      <div className="grid gap-5 md:grid-cols-3">
        {primaryFeatures.map((feature, index) => {
          ...
          return <motion.article key={feature.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className={`flex min-h-[285px] flex-col rounded-2xl border p-6 ${active ? "border-blue-400/70 bg-gradient-to-b from-blue-500/[0.12] to-white shadow-[0_0_32px_rgba(99,102,241,0.13)]" : "border-black/10 bg-white"}`}>
            <span className={`flex h-11 w-11 items-center justify-center rounded-xl border ${feature.accent === "orange" ? "border-blue-400/30 bg-blue-500/10 text-blue-400" : feature.accent === "cyan" ? "border-blue-400/30 bg-blue-500/10 text-blue-400" : "border-blue-400/30 bg-blue-500/10 text-blue-300"}`}><Icon className="h-5 w-5" /></span>
