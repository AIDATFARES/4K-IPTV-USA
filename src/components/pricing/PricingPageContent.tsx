...
const includedFeatures = [
  ...
  { icon: Headphones, title: "24/7 Support", text: "The IPTVDoor support team is here to help with setup and streaming questions." },
  { icon: Sparkles, title: "Fast Setup", text: "Your IPTVDoor details are sent after payment so you can start setting up quickly." },
  { icon: CreditCard, title: "TV Guide (EPG)", text: "Browse your channel schedule more easily with an electronic programme guide." },
];

const billingQuestions = [
  { question: "1. Which payment methods can I use?", answer: <>Available payment options are shown when you place your order. <Link className="font-semibold text-red-500 hover:text-red-400 transition-colors" href="/contact">Contact IPTVDoor support</Link> if you need help before purchasing.</> },
  { question: "2. Is my payment protected?", answer: "Please use the official IPTVDoor payment process and never share payment details through an unverified link or message." },
  { question: "3. Will my subscription renew automatically?", answer: <>Renewal details are provided when you order. If you have any questions about your subscription period or renewal, <Link className="font-semibold text-red-500 hover:text-red-400 transition-colors" href="/contact">contact support</Link> before your plan expires.</> },
  { question: "4. Can I change my plan or number of connections?", answer: <>Yes. <Link className="font-semibold text-red-500 hover:text-red-400 transition-colors" href="/contact">Contact the support team</Link> with your order email and the plan or connection change you need, and they will advise on the available options.</> },
];
    ...
    <section className="relative overflow-hidden bg-[#0c0f0f] py-16 sm:py-20 lg:py-24">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <header className="mx-auto max-w-3xl text-center">
          <span className="inline-flex rounded-full border border-red-400/35 bg-red-400/[0.06] px-4 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.18em] text-red-300">IPTVDoor</span>
          ...
          <p className="mx-auto mt-5 max-w-2xl text-sm leading-6 text-stone-400 sm:text-base">Choose the IPTVDoor plan that fits you, with secure checkout and helpful 24/7 support.</p>
          <p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-stone-500">Need help getting started? Visit our <Link className="font-semibold text-red-400 hover:text-red-300" href="/installation">installation guide</Link> or browse the <Link className="font-semibold text-red-400 hover:text-red-300" href="/channels">channel list</Link> before you order.</p>
        </header>
        ...
        <section className="mt-20 grid gap-8 rounded-3xl border border-white/10 bg-[#0b0f1c]/80 p-6 shadow-2xl lg:grid-cols-[1.2fr_0.8fr] lg:p-10">
          <div className="flex flex-col justify-center">
            <p className="text-sm font-bold uppercase tracking-[0.18em] text-red-300">Diamond plan</p>
            <h2 className="mt-3 text-3xl font-bold text-white sm:text-4xl">24 Months — our best subscription value</h2>
            <p className="mt-4 max-w-2xl leading-7 text-stone-400">Enjoy uninterrupted IPTVDoor streaming across supported devices, with fast activation, 4K and Full HD quality, a TV guide, and ongoing support.</p>
            <div className="mt-6 grid grid-cols-1 gap-3 text-sm text-stone-300 sm:grid-cols-2">
              {["4K & High Definition", "+50,000 Channels", "+120,000 Films & Series", "200,000+ VODs", "All supported devices", "Updated TV guide"].map((feature) => <p className="flex items-center gap-2" key={feature}><Check className="h-4 w-4 text-red-400" strokeWidth={3} />{feature}</p>)}
            </div>
          </div>
          <div className="rounded-3xl bg-white p-7 text-stone-900 border-2 border-red-600 shadow-2xl">
            ...
            <p className="mx-auto mt-3 w-fit rounded-full border border-red-200 bg-red-50 px-3.5 py-1 text-xs font-bold text-red-600">Best value</p>
            <ul className="mt-6 space-y-3 border-t border-stone-100 pt-5">{planFeatures.map((feature) => <li className="flex gap-2.5 text-sm font-medium text-stone-700" key={feature}><Check className="mt-0.5 h-4 w-4 shrink-0 text-red-500" strokeWidth={3} />{feature}</li>)}</ul>
        ...
        <section className="mx-auto mt-20 max-w-[1140px]">
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-5">
            {billingQuestions.map((item) => (
              <div className="flex flex-col rounded-2xl overflow-hidden border border-white/10 bg-[#0f1419]/90 shadow-xl hover:border-red-500/40 transition-colors" key={item.question}>
        ...
        <div className="mt-12 flex items-center justify-center gap-2 text-sm text-red-300"><ShieldCheck className="h-5 w-5 text-red-500" />Secure ordering and friendly IPTVDoor support.</div>
      </div>
    </section>
  );
}
