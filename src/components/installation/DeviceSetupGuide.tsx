...
const faqs = [
  ...
  { question: "4. How do I set up IPTVDoor using Xtream Codes API?", answer: "Choose Xtream Codes API in your player and enter the server URL, username, and password from your welcome email." },
    ...
    <section className="mb-16 md:mb-[100px]">
      ...
      <article className="mx-auto max-w-[940px] rounded-3xl border border-white/10 bg-[#121620]/90 p-7 sm:p-10 shadow-2xl">
        <div className="flex items-center gap-3 border-b border-white/10 pb-5">
          <DeviceIcon className="h-7 w-7 text-red-400" />
        ...
        <div className="mt-8 space-y-8">
          ...
          <div className="rounded-xl border-l-4 border-red-500 bg-red-950/30 px-4 py-3 text-sm leading-5 text-stone-300">{device.note}</div>
          ...
          <SetupStep number="4" title="Start Watching">Reopen the app to load your playlist and start watching IPTVDoor channels.</SetupStep>
        </div>
      </article>
      ...
      <div className="mx-auto mt-16 max-w-[1140px]">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {faqs.map((faq) => (
            <div className="flex flex-col rounded-2xl overflow-hidden border border-white/10 bg-[#0f1419]/90 shadow-xl hover:border-red-500/40 transition-colors" key={faq.question}>
...
function CodeLine({ label, text }: { label: string; text: string }) {
  return <div className="rounded-xl bg-black/40 border border-white/5 px-4 py-3 font-mono text-xs text-stone-400"><span className="font-bold text-red-400">{label}:</span> {text}</div>;
}
